...
def make_point_fig(prefix, group, lat0, lon0, location_name, out_dir):
    ...
    if all_nan:
        print(
            f"[{prefix}] no valid data at ({lat0}, {lon0}) -> nearest grid cell "
            f"({grid_lat:.2f}, {grid_lon:.2f}) is masked. Skipping."
        )
        return
    ...
    for i, v in enumerate(group["variables"]):
        ...
        # Do not place a separate legend in each subplot
        # ax.legend(...)
    ...
    # Remove any duplicated labels
    unique_legend = dict(zip(labels, handles))

    # One shared legend centered below all subplots
    fig.legend(
    ...
    out_path = os.path.join(
        out_dir,
        f"point_{slug}_{prefix}_temporal_evolution.png"
    )
...
def main():
    ...
    for prefix in args.groups:
        title = f"{location_name} ({prefix.upper()})" #Modified to add climate variables into title. 
        make_point_fig(prefix, GROUPS[prefix], args.lat, args.lon, title, args.out_dir)
